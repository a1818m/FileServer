...
public:
	...
	//解析接收
	int on_Register(const string& jsonrep)
	{
		Json::Reader reader;
		Json::Value	resp;
		...
		DataSource sql;
		sql.start();
		int n = sql.insert(password,username);
		if(n != 0)
			throw string("username or password is false");
		return 0;
	}
	...
	int on_login(const string& jsonrep)
	{
		...
		DataSource sql;
		sql.start();
		int n = sql.select(password,username);
		if(n != 0)
			throw string("username or password is false");
		sql.end();
		return 0;
	...
	virtual int Service()
	{
		...
		while(true)
		{
			...
			else if(rcv.type == MSG_REGISTER)
			{
				try{
					rc = on_Register(rcv.data);
				}
				catch(string e)
				{
					errorCode = -1;
